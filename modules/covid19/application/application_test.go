...
func TestNewCovid19App(t *testing.T) {
	h, err := newHandler(t)
	if err != nil {
	...
	if h != nil {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				got, err := NewCovid19App(tt.args.h)
				if (err != nil) != tt.wantErr {
					t.Errorf("NewCovid19App() error = %v, wantErr %v", err, tt.wantErr)
					return
				}
				if !reflect.DeepEqual(got.handler, tt.want.handler) {
					t.Errorf("NewCovid19App() = %v, want %v", got.handler, tt.want.handler)
				}
			})
		}
	}
